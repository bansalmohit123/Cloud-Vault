...
import { Upload, UploadIcon as IconUpload } from 'lucide-react';
  ...
  return (
    <div className="w-full" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
        className="p-4 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden border-2 border-white dark:border-gray-800"
      >
        <input
          {...getInputProps()}
          ref={fileInputRef}
        ...
        {files.length > 0 &&
          ...
          className={cn(
            "relative overflow-hidden z-40 bg-white dark:bg-neutral-900 flex flex-col items-start justify-start md:h-20 p-4 mt-4 w-full mx-auto rounded-md",
            "shadow-sm"
          )}
            >
        ...
        {!files.length && (
          <motion.div
            ...
            className="absolute opacity-0 border border-dashed border-sky-400 inset-0 z-30 bg-transparent flex items-center justify-center h-24 mt-4 w-full max-w-[8rem] mx-auto rounded-md"
          ></motion.div>
        )}
          </div>
        </div>
